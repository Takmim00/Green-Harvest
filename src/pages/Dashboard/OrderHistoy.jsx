...
const OrderHistory = () => {
  ...
  const fetchOrders = async (page = 1) => {
    try {
      setLoading(true);
      const res = await axios.get(`${API}?page=${page}&limit=${ordersPerPage}`);
      setOrders(res.data.results); // assuming API returns { results: [...], count: X }
      ...
      <div className="md:hidden space-y-3">
        {loading
          ? Array.from({ length: ordersPerPage }).map((_, i) => (
              <div key={i} className="border border-gray-200 rounded-lg p-4 animate-pulse h-28" />
            ))
          : orders.length === 0
              ...
              <React.Fragment key={page}>
                ...
                <button
                  ...
                  className={`w-9 h-9 rounded-full text-sm font-semibold transition-all ${
                    currentPage === page ? "bg-[#00B250] text-white shadow-sm" : "text-gray-700 hover:bg-gray-100"
                  }`}
                >
                  {page}
                </button>
